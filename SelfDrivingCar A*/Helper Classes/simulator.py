...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=True):
        self.env = env
        self.size = size if size is not None else ((2*self.env.grid_size[0] + 1) * self.env.block_size, (self.env.grid_size[1] + 2) * self.env.block_size)
    ...
    def render(self):
        ...
        self.screen.blit(self.font.render("Finish", True, self.colors['red'], self.bg_color), (150, (self.env.grid_size[1]+1+self.env.hang/6)*self.env.block_size))
        self.screen.blit(self.font.render("Start", True, self.colors['red'], self.bg_color), (150, (self.env.bounds[0]+self.env.hang)*self.env.block_size))
        ...
        for agent, state in self.env.agent_states.items():
            ...
            agent_pos = ((state['location'][0]+self.env.xadd )* self.env.block_size - agent_offset[0], (state['location'][1] + self.env.yadd )* self.env.block_size - agent_offset[1])
            if agent is self.env.primary_agent:
                    primaryagent=agent
                    print(agent_pos)
        ...
        self.screen.blit(self.font.render("Intro to AI", True, self.colors['black'], self.bg_color), (400, 300))
        self.screen.blit(self.font.render("Self-drive-car Simulation", True, self.colors['black'], self.bg_color), (350, 350))
        if self.env.done==True:
                self.screen.blit(self.font.render("Reached Goal!!! in "+repr(self.env.t)+" steps", True, self.colors['black'], self.bg_color), (350, 400))
        else:
                self.screen.blit(self.font.render("Simulation Running for "+repr(self.env.t)+" steps", True, self.colors['black'], self.bg_color), (350, 400))
                self.screen.blit(self.font.render("Agent action "+repr(primaryagent.action), True, self.colors['black'], self.bg_color), (350, 500))
