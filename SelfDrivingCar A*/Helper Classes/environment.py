...
class Environment(object):
    ...
    valid_actions = [None,'forward-2x','forward', 'left', 'right']
    ...
    def __init__(self, verbose=False, num_dummies=20, grid_size = (8, 17),config_file="default_config.txt",fixmovement=False):
        ...
        for i in range(self.num_dummies):
            self.create_agent(DummyAgent,location=self.dummy_agent_location_list[i],fixmovement=fixmovement)
...
class DummyAgent(Agent):
    ...
    def __init__(self, env,location=None,fixmovement=False):
        ...
        self.actionindex = 0
        self.next_action = random.choice(Environment.valid_actions[2:])
